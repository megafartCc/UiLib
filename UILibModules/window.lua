...
function Library:CreateWindow(opts)
    ...
    if chatEnabled and chatBtn then
        ...
        local function resolveChatProvider(rawProvider)
        ...
        bindTheme(chatMessagesScroll, "ScrollBarImageColor3", "Scroll")
        ...
        local function callChatFetch()
            if chatPollBusy or not chatOpen or not win.Visible or not keyGateUnlocked then
                return
            end
        ...
        onThemeChanged(refreshChatButtonVisual)

        setChatOpen = function(nextOpen)
        ...
        if chatStartsOpen then
            task.defer(function()
                if not win._destroyed and keyGateUnlocked and win.Visible then
                    setChatOpen(true)
                end
            end)
        end
